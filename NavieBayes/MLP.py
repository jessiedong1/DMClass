...
def main():
    ...
    #dataset_ori = nb.loadCSV(filename)
    #X,y = split_x_y(dataset_ori)
    ...
    #result = MLP(train_x, train_y, test_x, test_y,epoch,batch_size)
    #show_result(result)
    #Cross_Validation()
    #show_result(result1)
...
def MLP(train_x,train_y,test_x, test_y, epoch, batch_size):
    ...
    model.add(layer)
    model.add(layers.Dropout(0.3, noise_shape=None, seed=None))
    model.add(layers.Dense(250, activation="relu", kernel_regularizer=regularizers.l2(1e-10), activity_regularizer=regularizers.l1(1e-10)))
    ...
    model.add(layers.Dense(10, activation="relu", kernel_regularizer=regularizers.l2(1e-10),activity_regularizer=regularizers.l1(1e-10)))
    #model.add(layers.Dropout(0.2, noise_shape=None, seed=None))
    #model.add(layers.Dense(16, activation="relu", kernel_regularizer=regularizers.l2(1e-10),activity_regularizer=regularizers.l1(1e-10)))
    #model.add(layers.Dropout(0.05, noise_shape=None, seed=None))
    #model.add(layers.Dense(10))
    model.add(layers.Dense(1, activation="sigmoid"))
"""
